...
\begin{texexptitled}[breakable,center lower,enhanced jigsaw,middle=2mm,listing side comment,righthand width=6cm,compilable listing,run latex,run dvips,run ps2pdf,pdf comment,comment style={raster columns=1}]{Um documento \LaTeX{} mínimo}{exe_doc}
...
Pacotes no \LaTeX{} são extensões da linguagem que permitem que novas funcionalidades sejam utilizadas. Há muitos pacotes que são populares por suas funcionalidades. Por exemplo, caso seja necessário utilizar página nos modos retrato e paisagem no mesmo documento, pode-se utilizar o pacote {\tt rotating}. Neste caso, deve-se incluir o comando \mintinline{latex}{\usepackage{rotating}} no preâmbulo do documento. O pacote {\tt xcolor} é utilizado quando deseja-se utilizar cores diferentes no documento. Outro pacote útil, é o {\tt minted}, que permite listar códigos e destacar as palavras-chave de uma determinada linguagem, além de inserir numeração nas linhas de um código. Para a inserção de imagens vetoriais, pode-se utilizar os pacotes {\tt tickz} ou {\tt pstricks}, os quais provêm ambientes específicos para a programação da linguagem. Há também pacotes específicos para a confecção de tabelas, como o {\tt booktabs}. Neste documento, uma série de pacotes são carregados, a partir dos quais são mostrados os exemplos da linguagem \LaTeX{}. Especificamente para este propósito, utiliza-se o pacote {\tt tcolorbox}. Com este pacote, constroem-se as caixas coloridas, as dicas e os exemplos apresentados. No estilo do INPE, apresentado no Capítulo \ref{cap:parteIII}, muitos pacotes já são carregados por padrão, de forma que o usuário não precise se preocupar em descobrir qual pacote precisar ser carregado quando, por exemplo, precisar inserir legendas nas figuras que inserir. Para esta finalidade, pode-se utilizar os pacotes {\tt caption} e {\tt subcaption}.
...
Na primeira forma, basta utilizar a barra invertida ``\mintinline{latex}{\\}''. Na segunda, pode-se utilizar comandos específicos do \LaTeX{}. 
...
%\end{texexptitled}
\begin{texexptitled}[breakable,center lower,enhanced,middle=2mm,]{Marcação com o pacote \mintinline{latex}{cancel}}{exe_cancel}
...
Para alterar o tamanho de uma fonte localmente, basta fazer \mintinline{latex}{{\large palavra}}. Veja no Exemplo \ref{exe_tamfontefrase} a seguir:
...
\textit{The \colorbox{pink}{quick} \colorbox{magenta}{brown} fox jumps
\colorbox{green}{over} the lazy \colorbox{blue}{\color{white}{dog}}.}
\end{texexptitled}
...
Se for necessário, é possível definir qualquer cor utilizando códigos \textit{Hypertext Markup Language} (HTML), \textit{Red Green Blue} (RGB) ou \textit{Cyan Magenta Yellow Black} (CMYK) utilizando o comando \mintinline{latex}{\definecolor}.
...
No Exemplo \ref{exe_par1}, as sentenças inseridas pelo comando \mintinline{latex}{\lipsumsentence} são apresentadas em forma de bloco, formando um parágrafo contíguo. Para separar as sentenças geradas pelos comandos, pode-se simplesmente pular uma linha. Veja no Exemplo \ref{exe_par2} a seguir:
...
Semelhante ao Exemplo \ref{exe_par2} em que foi utilizado um espaço em braco para separar as sentenças no parágrafo, pode-se utilizar aspas duplas invertidas (\mintinline{latex}{\\}). Veja no Exemplo \ref{exe_par3} a seguir:
...
Outra forma se pular linhas, é através a utilização do comando \mintinline{latex}{\newline}. Veja o Exemplo \ref{exe_par4} e compare com os dois exemplos anteriores:

\begin{texexptitled}[breakable,enhanced,middle=2mm]{Parágrafos separados pelo comando (\mintinline{latex}{\newline})}{exe_par4}
\lipsumsentence[1-4]
\newline
\lipsumsentence[5-8]
\end{texexptitled}
...
\begin{texexptitled}[breakable,enhanced,middle=2mm]{Parágrafos centralizados \par Utilizando o ambiente \mintinline{latex}{center}}{exe_par5}
\begin{center}
\lipsumsentence[9-10] \\ 
...
Além de utilizar o ambiente \mintinline{latex}{center}, é possível utilizar o marcador \mintinline{latex}{centering}. Veja o Exemplo \ref{exe_par6} a seguir: 

\begin{texexptitled}[breakable,enhanced,middle=2mm]{Parágrafos centralizados \par Utilizando o marcador \mintinline{latex}{\centering}}{exe_par6}
\centering
...
No Exemplo \ref{exe_fig1}, observe que o marcador \mintinline{latex}{\includegraphics} aceita algumas opções que são delimitadas por um par de \mintinline{latex}{[]}'s (conchetes). Pode-se especificar, por exemplo, o tamanho da figura pode ser especificado com as opções \mintinline{latex}{width}, \mintinline{latex}{height} ou \mintinline{latex}{scale}.

\subsection{Formatos de Figuras}
\label{sec:form_figs}

Figuras podem ser incorporadas a partir de diferentes formatos em um documento \LaTeX{}. Os formatos preferenciais, entretanto, são o \textit{Portable Document Format} (PDF) e o \textit{Encapsulated PostScript} (EPS). Estes formatos são vetoriais e permitem, por exemplo, a impressão em alta resolução das figuras.

É possível converter formatos como o \textit{Portable Network Graphics} (PNG), \textit{Graphics Interchange Format} (GIF) e \textit{Joint Photographic Experts Group} (JPEG) para os formatos PDF e EPS. Para tanto, recomenda-se a utilização do programa \textit{Imagemagick} para esta conversão. Conversores online também podem ser utiizados para esta finalidade.
...
%%\end{figure}
%
%Para facilitar o desenho de diagramas, você pode utilizar o programa \href{http://latexdraw.sourceforge.net}{\textit{LaTeXDraw}} (em Java), disponível para os sistemas operacionais mais populares. Vários outros programas estão preparados para exportar gráficos para o formato \TeX{}.
%
%\begin{marker}
%A página \url{https://tug.org/PSTricks/main.cgi?file=index} apresenta muitos exemplos de desenhos feitos com o PSTricks.
%\end{marker}
%
...
O Bib\LaTeX{} é uma implementação mais moderna do software de gerenciamento de referências, sendo mais flexível do que o Bib\TeX{} por já estar no formato UNICODE, além de permitir a utilização de macros do próprio \LaTeX{} para a criação de novas classes de referências.
...
Segundo \citeonline{wedietal/2013}, a transformada rápida de Legendre
...
Grandes bases de referências podem ser organizadas e incrementadas com o auxílio de software como o \textit{BibDesk} (apenas para o Mac OS), \textit{JabRef}, \textit{Mendeley} e \textit{Zotero}. Estes \textit{software} são bastante úteis pois permitem a organização das referência e dos seus metadados.
...
Atualmente a maioria das revistas fornecem as referências em diferentes formatos. Se houver dificuldade em obter o formato Bib\TeX{} de uma referência, pode-se utilizar o site \href{https://doi2bib.org}{\textit{DOI2Bib}} para se obter a referência com os campos corretos.
